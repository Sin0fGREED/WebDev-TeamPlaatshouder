...
public static class NotificationsApi
{
    public static RouteGroupBuilder MapNotificationsApi(this RouteGroupBuilder g)
    {
        ...
        g.MapGet(
                ...
                ) =>
                {
                    ...
                    if (userId.HasValue && !includeRead)
                    {
                        ...
                        q = q.Where(n =>
                            !db.NotificationDismissals.Any(nd =>
                                nd.NotificationId == n.Id && nd.UserId == uid && nd.IsRead
                            )
                        );
                    }

                    var items = await q.OrderByDescending(n => n.Timestamp)
                        ...
                        .Select(n => new NotificationDto(
                            ...
                            userId.HasValue
                                && db.NotificationDismissals.Any(nd =>
                                    nd.NotificationId == n.Id
                                    && nd.UserId == userId.Value
                                    && nd.IsRead
                                )
                        ))
                        .ToListAsync();
        ...
        g.MapDelete(
                "{id}",
                async (Guid id, AppDbContext db, ClaimsPrincipal user) =>
                {
                    var userIdString =
                        user.FindFirst("sub")?.Value
                        ?? user.FindFirst(System.Security.Claims.ClaimTypes.NameIdentifier)?.Value;
                    if (!Guid.TryParse(userIdString, out var userId))
                    {
                        return Results.BadRequest(
                            new { error = "Cannot resolve user id from token." }
                        );
                    }
                    ...
                    var dismissal = await db.NotificationDismissals.FirstOrDefaultAsync(nd =>
                        nd.NotificationId == id && nd.UserId == userId
                    );
