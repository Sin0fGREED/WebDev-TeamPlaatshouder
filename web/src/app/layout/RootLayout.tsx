...
const navLink = ({ isActive }: { isActive: boolean }) =>
  `card-hover flex items-center gap-3 rounded-xl px-3 py-2 overflow-hidden transition-all duration-150 ease-in-out ${
    isActive
      ? "hover:text-white bg-blue-500 dark:bg-white-700/15 text-white dark:bg-white/10 dark:text-white shadow-sm transition-colors duration-500"
      : "hover:bg-blue-900/10 dark:hover:bg-white/5 text-gray-700 dark:text-gray-200 transition-colors duration-500"
  }`;
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="flex">
        {/* Sidebar */}
        <aside
          className={`hidden md:flex sticky top-0 self-start ${
            collapsed ? "w-20" : "w-64"
          } h-screen flex-col gap-6 border-r border-border/60 bg-[rgb(var(--panel))] p-4 overflow-y-auto transition-width duration-200`}
        >
        ...
        <main className="flex-1">
          ...
          <NavLink
            to="/events/new"
            className="fixed right-4 top-4 z-50 inline-flex items-center gap-2 rounded-full px-4 py-2 btn-accent shadow-lg sm:hidden transform transition-all duration-150 ease-out hover:-translate-y-1 hover:scale-105 hover:shadow-xl"
