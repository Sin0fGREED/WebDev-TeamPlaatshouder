...
export default function NotificationIcon() {
  ...
  useEffect(() => {
    ...
    async function ensure() {
      if (hub.state === "Disconnected") {
        try { await hub.start(); } catch { }
      }
    }

    ensure();

    // load unread notifications (first page) and set unread count
    async function loadInitial() {
      try {
        const token = localStorage.getItem("token");
        const res = await fetch(`${import.meta.env.VITE_API_BASE}/api/notifications?page=1&pageSize=20&includeRead=false`, {
  ...
  // All notifications are shown on the dedicated page.
                ...
                <div key={n.id} className="p-3 hover:bg-gray-50 dark:hover:bg-gray-700 border-b border-border/40">
                  <div className="flex items-start gap-3">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        // mark as read on server for this user
                        try {
                          const token = localStorage.getItem("token");
                          const res = await fetch(`${import.meta.env.VITE_API_BASE}/api/notifications/${n.id}`, {
                            method: "DELETE",
                            headers: token ? { Authorization: `Bearer ${token}` } : undefined,
                          });
                        ...
                        // remove from dropdown (unread list) only
